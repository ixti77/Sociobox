...
final class ProfileContentProvider<Store>: ProfileContentProviderProtocol where Store: PreferencesStoreProtocol {
	...
	var feedView: AnyView {
		privacyLevel > store.feedPreference ? AnyView(HistoryFeedView(posts: user.historyFeed)) : AnyView(RestrictedAccessView())
	}
	
	var friendsView: AnyView {
		privacyLevel > store.friendsListPreference ? AnyView(UsersView(title: "Friends", users: user.friends)) : AnyView(EmptyView())
	}
}
